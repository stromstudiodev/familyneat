...
// --- Selección de emoji de animal en registro de hijo ---
  const selectorAvatarHijo = document.getElementById('selector-avatar-hijo');
  selectorAvatarHijo?.addEventListener('click', (e) => {
    const btn = e.target.closest('.chip-emoji');
    if (!btn) return;
    selectorAvatarHijo.querySelectorAll('.chip-emoji').forEach(b => b.classList.remove('activo'));
    btn.classList.add('activo');
    document.getElementById('input-avatar-hijo').value = btn.dataset.emoji;
  });

  // Modificación en registroHijo para incluir el avatar elegido
  async function registrarHijo(nombre, pin, codigo) {
    const familia = await buscarFamiliaPorCodigo(codigo);
    if (!familia) return null;

    const secretoHash = await hashTexto(pin);
    const avatarEmoji = document.getElementById('input-avatar-hijo')?.value || '🐶';
    const familiaRef = db.collection('familias').doc(familia.id);
    ...
    const miembroRef = await familiaRef.collection('miembros').add({
      ...
      secretoHash,
      aprobado: false,
      avatar: avatarEmoji, // Guardamos el emoji de animal
      esEmoji: true,
      saldoDinero: 0,
      saldoPuntos: 0,
      creadoEn: firebase.firestore.FieldValue.serverTimestamp(),
