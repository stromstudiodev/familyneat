...
(() => {
  ...
  async function sembrarDatosDemo(familiaRef, miembroId, nombreHijo) {
    ...
    lote.set(tarea1, {
      ...
      dinero: 0.5, puntos: 10, estado: 'pendiente', fecha: 'Hoy', creadoEn: ahora,
    });
    const tarea2 = familiaRef.collection('tareas').doc();
    lote.set(tarea2, {
      titulo: 'Ordenar el baño', estancia: 'bano', miembroId, miembroNombre: nombreHijo,
      dinero: 1, puntos: 15, estado: 'revision', fecha: 'Ayer', creadoEn: ahora,
    });
    ...
    contenedor.innerHTML = hijosPendientes.map((h) => `
      <div class="tarjeta-aprobacion">
        <div class="fila-aprobacion">
          ...
          <div class="info-hijo">
            <strong>${h.nombre}</strong>
            <span>Quiere unirse como hijo/a</span>
          </div>
        </div>
  ...
  function iniciarSesionPadre() {
    ...
    if (textoCodigo) textoCodigo.textContent = `NEAT-${codigo}`;

    const fRef = db.collection('familias').doc(familiaId);

    ESTADO.listenersActivos.push(
      fRef.collection('miembros').where('rol', '==', 'hijo').onSnapshot((snap) => {
        const hijos = snap.docs.map((d) => ({ id: d.id, ...d.data() }));
        ESTADO.hijosFamiliaCache = hijos;
        renderizarHijosInicio(hijos.filter((h) => h.aprobado));
        renderizarAprobaciones(hijos.filter((h) => !h.aprobado));
        poblarSelectHijos(hijos.filter((h) => h.aprobado));
      }),
  ...
  function inicializarFormulariosBasicos() {
    ...
    // Registro hijo → busca la familia por código y crea el perfil (pendiente de aprobación)
  ...
  function inicializarModalesFamilia() {
    ...
    document.getElementById('form-unirse-familia')?.addEventListener('submit', async (evento) => {
      ...
      boton.disabled = true;
      try {
        const exito = await unirsePadreAFamilia(codigo);
        if (!exito) {
          mostrarToast('No encontramos ninguna familia con ese código', 'error');
          return;
        }
        modalUnirse.close();
        mostrarToast('¡Te has unido a la familia!', 'exito');
        iniciarSesionPadre();
        irAPantalla('pantalla-padre-inicio');
      } catch (error) {
        console.error(error);
  ...
  function inicializarLoginPerfiles() {
    ...
    formAuth?.addEventListener('submit', async (evento) => {
      ...
      if (esHijo && !miembro.aprobado) {
        mostrarToast('Tu cuenta todavía está pendiente de aprobación', 'error');
        return;
      }
  ...
  function abrirModalLogin(miembro) {
    ...
    if (miembro.rol === 'hijo') {
      ...
      modoPin.querySelectorAll('.pin-box').forEach((c) => { c.value = ''; c.classList.remove('lleno'); });
  ...
  function inicializarPanelPadre() {
    document.getElementById('btn-copiar-codigo')?.addEventListener('click', () => {
      const texto = document.getElementById('texto-codigo-familia')?.textContent || '';
      navigator.clipboard?.writeText(texto).then(() => mostrarToast('Código copiado', 'exito'))
    ...
    document.getElementById('form-nueva-tarea')?.addEventListener('submit', async (evento) => {
      ...
      try {
        await crearTareaNueva({
          ...
          fecha: document.getElementById('fecha-nueva-tarea').value.trim(),
        });
    ...
    document.getElementById('lista-aprobaciones')?.addEventListener('click', async (evento) => {
      ...
      try {
        if (boton.dataset.accion === 'aprobar-hijo') {
          ...
          mostrarToast('Hijo/a aprobado/a', 'exito');
        } else {
